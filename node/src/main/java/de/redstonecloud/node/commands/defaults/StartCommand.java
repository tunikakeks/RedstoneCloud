...
public class StartCommand extends Command {
    ...
    protected void onCommand(String[] args) {
        ...
        Template template = RedstoneNode.getInstance().getServerManager().getTemplate(args[0]);
        if (template == null) {
        ...
        if (amount != 1 && newId == -1) {
            for (int i = 0; i < amount; i++) {
                RedstoneNode.getInstance().getServerManager().startServer(template);
            }
        } else RedstoneNode.getInstance().getServerManager().startServer(template, newId);
