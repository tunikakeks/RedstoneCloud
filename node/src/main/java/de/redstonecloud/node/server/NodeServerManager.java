...
public class NodeServerManager {
    ...
    private NodeServerManager() {
        log.info("Initializing NodeServerManager");
        loadServerTypes();
        loadTemplates();
        log.info("NodeServerManager initialized with {} types and {} templates",
    ...
    public Server startServer(Template template, Integer id) {
        ...
        // Start server immediately (no scheduler in node)
        new Thread(() -> {
            try {
                Thread.sleep(1000);
                server.start();
        ...
        log.info("Server {} created and started", server.getName());
        return server;
    }
