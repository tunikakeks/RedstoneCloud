...
import de.redstonecloud.node.scheduler.task.TaskHandler;
...
public class ServerLogger extends Thread {
    ...
    public void run() {
        ...
        this.writerTask = RedstoneNode.getInstance().getScheduler().scheduleRepeatingTask(() -> {
            try {
    ...
    public void cancel() {
        ...
        if (logToConsole) RedstoneNode.getInstance().setCurrentLogServer(null);
